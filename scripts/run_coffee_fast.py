...
def run_fast(train_dir: Path, test_dir: Path):
    ...
    train_sub = make_subset(train_dataset, max_per_class=200)
    test_sub = make_subset(test_dataset, max_per_class=50) if test_dataset is not None else None

    train_loader = DataLoader(train_sub, batch_size=16, shuffle=True)
    test_loader = DataLoader(test_sub, batch_size=16, shuffle=False) if test_sub is not None else None
