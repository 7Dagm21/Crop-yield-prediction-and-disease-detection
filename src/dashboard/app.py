...
def render_yield_prediction() -> None:
    ...
    if CROP_YIELD_DATA_PATH is not None and not CROP_YIELD_DATA_PATH.exists():
        st.info("Using built-in demo yield data because the cloud deployment does not include the local training CSV.")
...
def render_disease_detection() -> None:
    ...
    try:
        ...
        st.error(str(exc))
        return
    if result.get("model_source") == "missing":
        st.warning(result.get("message", "No disease model is available in this deployment."))
